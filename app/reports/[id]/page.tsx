...
import { serverAnalysisAPI } from "@/lib/api/server"
...
async function ReportAnalysisContent({ documentId }: { documentId: string }) {
  // Require authentication
  await requireAuth()
  
  // Fetch analysis data on the server
  const analysisResult = await serverAnalysisAPI.getAnalysis(documentId)
      ...
      <div className="lg:col-span-2 flex flex-col gap-6">
        ...
        <div className="rounded-xl border border-border bg-card p-6">
          ...
          <div className="flex justify-center">
            <SentimentScore
              score={intelligence_hub?.sentiment.score || 0}
        ...
        {analysisResult.retrieval_scores && analysisResult.retrieval_scores.length > 0 && (
          <div className="rounded-xl border border-border bg-card p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
...
export default async function ReportAnalysisPage({ params }: ReportAnalysisPageProps) {
    ...
    <AppShell>
      <div className="p-6 lg:p-8 max-w-7xl mx-auto">
        <Suspense fallback={
          ...
          <ReportAnalysisContent documentId={documentId} />
        </Suspense>
      </div>
    </AppShell>
  )
